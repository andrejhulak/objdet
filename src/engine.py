...
# @torch.no_grad()
# def test_single_image(model, postprocessors, input, device):
#   model.eval()
...
#     cv2.rectangle(img_cv, (x0, y0), (x1, y1), color=(0, 0, 255), thickness=2)
#     label_text = f"{pred['category_id']}:{pred['score']:.2f}"
...
def test_single_image(model, postprocessors, path, device):
  ...
  top_preds = sorted(final_res, key=lambda x: x['score'], reverse=True)[:20]
  for i, pred in enumerate(top_preds):
    print(f"Pred {i+1}: score={pred['score']:.3f}, class={pred['category_id']}, bbox={pred['bbox']}")
